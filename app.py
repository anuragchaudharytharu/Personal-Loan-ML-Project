...
"""
Simple Loan Approval Prediction UI
Run with: streamlit run app.py
"""
...
if pipeline is None:
    st.error("⚠️ Model not found. Please run 'python main.py' first.")
    st.stop()
...
with col1:
    ...
    income = st.number_input("Annual Income ($)", min_value=0, max_value=500000, value=75000, step=5000)
    experience = st.number_input("Work Experience (years)", min_value=0, max_value=50, value=5)
    family = st.number_input("Family Size", min_value=1, max_value=10, value=2)
    
    education = st.selectbox(
        "Education Level",
        options=[1, 2, 3],
        format_func=lambda x: {1: "Graduate", 2: "Post Graduate", 3: "Advanced"}[x]
    )

with col2:
    st.subheader("💰 Financial Info")
    mortgage = st.number_input("Mortgage Value ($)", min_value=0, max_value=500000, value=0, step=10000)
    securities = st.number_input("Securities Amount ($)", min_value=0, max_value=200000, value=0, step=5000)
    cd_amount = st.number_input("CD Amount ($)", min_value=0, max_value=100000, value=0, step=5000)
    
    st.subheader("📱 Banking")
    online = st.selectbox("Online Banking", options=[1, 0], format_func=lambda x: "Yes" if x == 1 else "No")
    credit_card = st.selectbox("Credit Card", options=[1, 0], format_func=lambda x: "Yes" if x == 1 else "No")
    securities_acc = st.selectbox("Securities Account", options=[0, 1], format_func=lambda x: "Yes" if x == 1 else "No")
    cd_acc = st.selectbox("CD Account", options=[0, 1], format_func=lambda x: "Yes" if x == 1 else "No")
...
if st.button("🔮 Predict Loan Approval", type="primary", use_container_width=True):
    # Prepare features
    features = [
        age, income, experience, family, education,
